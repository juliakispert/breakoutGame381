...
public class Ball extends Ellipse {
    ...
    /**
     * This method updates the ball's position, and also deals with what happens
     * when the ball
    ...
    public void updatePosition(CanvasWindow canvas, double dt, BrickManager brickManager, Paddle paddle) {
        ...
        if (!hitPaddle(canvas, paddle)) {
            hitBricks(canvas, brickManager);
        }
    }

    /**
     * Given a canvas, this method changes the direction of the ball if it doesn't
     * hit the top,
     * left and right wall.
     */
     ...
     */
    private void hitBricks(CanvasWindow canvas, BrickManager brickManager) {
        ...
        if (brickManager.checkIfBrick(topLeft, topRight)) {
            speedY = -speedY;
            brickManager.removeBricks(topLeft, topRight);
        }

        if (brickManager.checkIfBrick(bottomLeft, bottomRight)) {
            speedY = -speedY;
            brickManager.removeBricks(bottomLeft, bottomRight);
        }

        if (brickManager.checkIfBrick(topLeft, bottomLeft)) {
            speedX = -speedX;
            brickManager.removeBricks(topLeft, bottomLeft);
        }

        if (brickManager.checkIfBrick(topRight, bottomRight)) {
            speedX = -speedX;
    ...
    /**
     * Given a canvas, this returns true if either corner of the ball touches the
     * paddle.
    ...
    private boolean hitPaddle(CanvasWindow canvas, Paddle paddle) {
        ...
        if (canvas.getElementAt(bottomLeft) instanceof Paddle || canvas.getElementAt(bottomRight) instanceof Paddle
                || canvas.getElementAt(topLeft) instanceof Paddle || canvas.getElementAt(topRight) instanceof Paddle) {
            speedY *= -1;
            return true;
        } else {
            return false;
